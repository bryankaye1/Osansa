function [data,jmax, intb] = spc_2_his(tmini,tmaxi,file_name,pth_sdt,thresh,thrt,DesPho,tsm)
...
if length(sild) == 1
    ...
    if thrt == 0 %%%group pixels by intensity hi to lo
        for j=1:128
            for k = 1:128*tsm
        ...
        if thrt==-1
            ...
            the=min(min(pmt))-1;
            g=0;
            while the < max(max(pmt))
                g = g+1;
                ths = the+1;
                pgroup = 1;
                while sum(sum(pgroup)) < DesPho && the < max(max(pmt))
                    the = the + 1;
                    pmigr = pmt>=ths;
                    pmile = pmt<=the;
