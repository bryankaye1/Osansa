...
for el = 1
    ...
    comment = 'find short lifetime m5';
    tfw = .6;%(el-1)*.1;
    tbac = .2;
    tsm=1;
    ...
    while jind < jmax%for jind = 1:jmax
        ...
        for expt = 1:exptmax
            for cindex = 1:cyclesmax
                ...
                irfname = 'irf-428pm.sdt';
                data_shift_name = 'm0_s1.sdt';
                str(1).name = 'm5_s1';
                %                 if el <10
                ...
                %str(1).name = strcat('m',num2str(idivide(int32(el),int32(3))),...
                    %'t-spot',num2str(mod(el,3)+1));
                ...
                pth_irf = 'C:\Users\Bryan\Documents\MATLAB\Bayes_2014\data\2014-11-10\'; %Type the file path of the Instrument Response Function (IRF) here.
                ...
                w1step = .005; w1min= 1.45; w1max = 1.45;%1.6 used for extract 8/27 and 9/5 (actual 9/5 is 1.59) %1.5 used for taxol extract; %.8-2 1.05 %w1min must be an integer multiple of w1step.
                w2step = .005; w2min =  3.71; w2max = 3.71;%3.745 usd for 8/27 E %3.87 used for taxol extract; %3.81 was found for 8/25 b80 and 8/24 extract
                ...
                ngr = 64;%jind*100000;
                if jind == 1
                    [pmat,jmax,nphi] = spc_2_his_temp(tmini,tmaxi,dataname,pth_data,ngr,tsm);
                    p = pmat(jind,:);
                else
                    p = pmat(jind,:);
                end
                ...
                input(cindex,expt,jind).ngr = ngr;  %if input.thresh/thrt is 2, DesPho sets the number of photons
                input(cindex,expt,jind).ngr = nphi;  
