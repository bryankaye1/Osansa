    ...
    while jind < jmax%for jind = 1:jmax
        ...
        for expt = 1:exptmax
            if expt == 1
                 ...
                 str(1).name = strcat('m1-s1-spot',num2str(el),'.sdt');
...
%                 str(6).name = 'te1-spot6.sdt';
            ...
            for cindex = 1:cyclesmax
                ...
                if jind == 1
                [pmat,jmax,intb] = read_imagethr_14nsP(tmini,tmaxi,dataname,pth_data,imthr,thrt,DesPho);
                p = pmat(jind,:);
                else
                      p = pmat(jind,:);
                end
