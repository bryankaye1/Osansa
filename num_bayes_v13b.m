...
pth_irf = 'Z:\bkaye\Bayes_2013\data\2014-2-13\'; %Type the file path of the Instrument Response Function (IRF) here.
irfname = 'irf-807pm.sdt';
...
pth_data_for_shift = 'Z:\bkaye\Bayes_2013\data\2014-2-27\';
...
if isequal(new_filenames,old_filenames) && sysinfo == 0;
    ...
    sfrac = .2; shiftmin = -7; shiftmax = -1;
    w2step = 0.01; w2min = 3.6; w2max = 3.8;
...
pth_data = 'Z:\bkaye\Bayes_2013\data\2014-2-27\';
dataname = 'm0-e1-s1-spot1.sdt';
image = 0;

if image ==1
    tmini = floor(tmini/16); % Added only because IRF was taken with 4096 bins and image with 256
    tmaxi = floor(tmaxi/16);
    bins = floor(bins/16);
    brem = floor(brem/16);
    binskeep = bins-brem;    
    
    [p] = read_image_14ns(tmini,tmaxi,dataname,pth_data);
else
    [p] = read_image_14ns(tmini,tmaxi,dataname,pth_data);% p is the histogram data. dataname is the name of the data file
end
...
for l1 = 1:2
    ...
    if l1 == 1
        w1step = .02; w1min= .3; w1max = 1; %w1min must be an integer multiple of w1step.
        w2step = .02; w2min = 3; w2max = 4;
        fracstep = 0.02;
    ...
    sl = 2; sr = 2; % Sets how many data points to the left and right of the threshold to scan on the next loop;
