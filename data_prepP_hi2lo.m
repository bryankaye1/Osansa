...
for el = 0:11
    ...
    comment = 'm0t-m3t spot1-3 FF vs int 100k pho with lifetmes from 6/13: w1=1.5, w2=3.87';
    tfw = .6;%(el-1)*.1;
    tbac = .2;
    ...
    while jind < jmax%for jind = 1:jmax
        ...
        for expt = 1:exptmax
            if expt == 1
                 ...
                 %str(1).name = 'm6.sdt';
                str(1).name = strcat('m',num2str(idivide(int32(el),int32(3))),'t-spot',num2str(mod(el,3)+1),'.sdt');
...
%                 str(6).name = 'te1-spot6.sdt';
            ...
            for cindex = 1:cyclesmax
                ...
                w2step = .005; w2min =  3.87; w2max = 3.87;%3.745 usd for 8/27 E %3.87 used for taxol extract; %3.81 was found for 8/25 b80 and 8/24 extract
                ...
                DesPho = 200*10^3;%jind*100000;
                
                if jind == 1
                [pmat,jmax,intb] = read_imagethr_14nsP_hi2lo(tmini,tmaxi,dataname,pth_data,imthr,thrt,DesPho);
                p = pmat(jind,:);
                else
                      p = pmat(jind,:);
                end
